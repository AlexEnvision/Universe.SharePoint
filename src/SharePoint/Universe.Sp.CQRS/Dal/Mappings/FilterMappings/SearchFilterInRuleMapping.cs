...
namespace Universe.Sp.CQRS.Dal.Mappings.FilterMappings
{
    ...
    /// </summary>
    internal class SearchFilterInRuleMapping : AutoMap<InConfiguration, CamlChainRule>
    {
        protected override void Configure(IMappingExpression<InConfiguration, CamlChainRule> config)
        {
            base.Configure(config);
            config.Map(x => x.RuleBody, x => CamlHelper.GetInTexts(this.GetFieldName(x.LeftOperand), this.GetValue(x.RightOperand)));
        }
        ...
        private List<string> GetValue(IArgumentConfiguration operand)
        {
            var valueConfig = operand as ValueArgumentConfiguration;
            var value = valueConfig?.Expression.Replace("'", "").Split(';').ToList();
            return value;
        }
    }
}